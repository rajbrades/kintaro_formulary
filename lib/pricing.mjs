...
export function isTestosterone(row) {
  return /^testosterone/i.test(row.product_name.trim());
}
...
export function orderFulfillment(row, shippingMethod = "standard") {
  ...
  const included = includesColdShipping(row);
  return {
    shippingMethod: included ? "coldOvernight" : shippingMethod,
    shippingCost: included ? 0 : PRICING_MODEL.shipping[shippingMethod],
    processingFee: included ? PRICING_MODEL.glp1ProcessingFee : 0,
    suppliesIncluded: included,
  };
}
...
export function suggestedRetail(productCost, months, consultCost, fulfillmentCost = PRICING_MODEL.shipping.standard) {
  const feeRate = PRICING_MODEL.medicalBranchRate + PRICING_MODEL.merchantRate;
  const orderCost = productCost * months + consultCost + fulfillmentCost;
    ...
    plans[consult] = Object.fromEntries(
      PRICING_MODEL.supplyMonths.map((months) => [
        months,
        {
          ...
          suggested_retail: suggestedRetail(product.amount, months, cost, standard.shippingCost + standard.processingFee),
          two_day_suggested_retail: suggestedRetail(product.amount, months, cost, twoDay.shippingCost + twoDay.processingFee),
          overnight_suggested_retail: suggestedRetail(product.amount, months, cost, overnight.shippingCost + overnight.processingFee),
        },
      ])
    );
  }
