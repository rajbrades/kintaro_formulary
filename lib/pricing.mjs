export const PRICING_MODEL = Object.freeze({
  ...
  shipping: Object.freeze({
    // Standard = 2-day service (the default). Priority = overnight service.
    standard: Object.freeze({ cost: 15, retail: 15 }),
    priority: Object.freeze({ cost: 20, retail: 25 }),
    // Cold-chain overnight for injectable Tirzepatide/Semaglutide (own rate).
    coldOvernight: Object.freeze({ cost: 35, retail: 35 }),
  }),
...
export function orderFulfillment(row, shippingMethod = "standard") {
  ...
  const shipping = PRICING_MODEL.shipping[cold ? "coldOvernight" : shippingMethod];
  return {
...
export function monthlyProductCost(row) {
  ...
  if (row.wholesale_basis === "unknown") return null;
  if (row.wholesale_basis === "per_unit") {
    return { amount: wholesale * PRICING_MODEL.daysPerMonth, basis: `$${wholesale} / unit × 30` };
  }
  if (row.wholesale_basis === "per_30_day") {
    return { amount: wholesale, basis: "30-day supply" };
  }
  return { amount: wholesale, basis: `${row.package || "1 package"} / month (assumed)` };
}
...
export function enrichPricing(row) {
  ...
  const priority = orderFulfillment(row, "priority");
  const plans = {};
    ...
    plans[consult] = Object.fromEntries(
      PRICING_MODEL.supplyMonths.map((months) => [
        months,
        {
          ...
          priority_suggested_retail: suggestedRetail(product.amount, months, cost, priority.shippingRetail + priority.processingFee),
        },
      ])
    );
  }
