...
/** How each wholesale quote is expressed by its source price sheet. */
export const WHOLESALE_BASES = ["per_unit", "per_30_day", "per_package", "unknown"];
...
export function loadRows(text) {
  ...
  for (let i = 1; i < records.length; i++) {
    ...
    if (!basisSet.has(row.wholesale_basis)) {
      throw new Error(`line ${line}: wholesale_basis "${row.wholesale_basis}" must be ${WHOLESALE_BASES.join("/")}`);
    }
    if (row.wholesale_cost && row.wholesale_basis === "unknown") {
      throw new Error(`line ${line}: priced row cannot have unknown wholesale_basis`);
    }
    rows.push(row);
  }
  return rows;
