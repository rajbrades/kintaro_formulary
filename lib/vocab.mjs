...
// ── shared ordering: group similar items (product name → strength → pharmacy) ──
/** Leading numeric value of a strength string, for dose ordering. */
export function strengthNumber(s) {
  const m = String(s || "").match(/[\d.]+/);
  return m ? parseFloat(m[0]) : Infinity;
}
/** Default presentation order so variants of a product cluster together. */
export function compareProducts(a, b) {
  let d = String(a.product_name).toLowerCase().localeCompare(String(b.product_name).toLowerCase());
  if (d) return d;
  d = strengthNumber(a.strength) - strengthNumber(b.strength);
  if (d) return d;
  return String(a.pharmacy).localeCompare(String(b.pharmacy));
}
